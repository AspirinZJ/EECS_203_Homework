...
void CPlot::DrawAxis(IplImage *image)
{
	...
	{
		_snprintf(text, sizeof(text) - 1, "%.3f",
				  i * y_scale_pixes / this->y_scale + this->y_min);                      // add by zhyh2010
	...
	{
		_snprintf(text, sizeof(text) - 1, "%.3f", -i * y_scale_pixes / this->y_scale + this->y_min);
		cvPutText(image, text, cvPoint(bs / 5, x_axis_pos + i * y_scale_pixes), &font, this->axis_color);
	}

	// x_max
	_snprintf(text, sizeof(text) - 1, "%.3f", this->x_max);
	cvPutText(image, text, cvPoint(w - bs / 2 - strlen(text) * chw, x_axis_pos), &font, this->text_color);

	// x min
	_snprintf(text, sizeof(text) - 1, "%.3f", this->x_min);
	...
	double x_scale_pixes = max(gw / ceil(x_max - x_min), chw * 4.0);                     // modify by zhyh2010
		...
		for (size_t j = 0; j < this->dataset[i].size(); j++)
		{
			...
			else if (this->lineTypeSet[i].type == '*')
			{
				...
				cvLine(image, cvPoint(current_point.x - slope_radius, current_point.y - slope_radius),
					   cvPoint(current_point.x + slope_radius, current_point.y + slope_radius), lineTypeSet[i].color, 1,
					   8);
			...
			else if (this->lineTypeSet[i].type == 's')
			{
				cvRectangle(image, cvPoint(current_point.x - slope_radius, current_point.y - slope_radius),
							cvPoint(current_point.x + slope_radius, current_point.y + slope_radius),
							lineTypeSet[i].color, 1, 8);
				if (lineTypeSet[i].is_need_lined == true)
				{
					if (j >= 1)
					{
